...
async function queryCore() {
  ...
  try {
    ...
    const r2 = await pool.query("SELECT COUNT(*)::int AS cnt FROM integration.sync_events WHERE processed = FALSE");
    const r3 = await pool.query("SELECT COUNT(*)::int AS cnt FROM integration.sync_events WHERE processed = TRUE");
